        ...
        <div class="flex-1">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <small class="ml-2 text-sm text-gray-600">{{ dayjs(chat.created_at).fromNow() }}</small>
                    <small v-if="chat.created_at !== chat.updated_at" class="text-sm text-gray-600">&middot; edited</small>
                </div>
                <Dropdown v-if="chat.user.id === $page.props.auth.user.id">
                    <template #trigger>
                        <button>
...
<!--                        </DropdownLink>-->
            ...
            <form v-if="editing" @submit.prevent="form.put(route('chats.update', chat.id), { onSuccess: () => editing = false })">
            ...
            <p v-else class="mt-4 text-lg text-gray-900">{{ chat.message }}</p>
        </div>
    </div>
</template>
